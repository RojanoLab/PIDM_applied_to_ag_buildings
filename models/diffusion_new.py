...
class ConditionalModel(nn.Module):
    def __init__(self, config):
        ...
        self.num_res_blocks = num_res_blocks
        # self.resolution = resolution
        curr_res = list(self.resolution)  # ✅ ahora es [H, W]
        self.in_channels = in_channels
        ...
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks):
                block.append(ResnetBlock(in_channels=block_in,
                                         ...
                                         dropout=dropout))
                block_in = block_out
                if curr_res in attn_resolutions:         # NO ATTENTION  #EFECTIVAMENTE NO HAY BLOQUE DE ATENCION, DICE QUE SI LA RESOLUCION ES 16X16 SE APLICA ATT, PERO EN NINGUNA CAPa de DWS SE CUMPLE.
                    attn.append(AttnBlock(block_in))                   
            down = nn.Module()
            down.block = block
            down.attn = attn
            if i_level != self.num_resolutions-1:
                down.downsample = Downsample(block_in, resamp_with_conv)
                # curr_res = curr_res // 2
                curr_res[0] = curr_res[0] // 2  # reduce altura
                curr_res[1] = curr_res[1] // 2  # reduce ancho
            self.down.append(down)
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if i_level != 0:
                up.upsample = Upsample(block_in, resamp_with_conv)
                # curr_res = curr_res * 2
                curr_res[0] = curr_res[0] * 2  # aumenta altura
                curr_res[1] = curr_res[1] * 2  # aumenta ancho
    ...
    def forward(self, x, t, dx=None):
       ...
       f"Input image shape {(x.shape[2], x.shape[3])} doesn't match expected {self.resolution}"   #EL MODELO ENTIENDE ALTO LUEGO ANCHO, SE INVIERTE A COMO VIENE EN IMAGE2NPY E IMAGE_RESIZER
        ...
        # Inicializamos h con combine_conv (IMPORTANTE)
    ...
    #     # RuntimeError: Given groups=1, weight of size [64, 192, 1, 1], expected input[12, 128, 256, 256] to have 192 channels, 
    #     # but got 128 channels instead.
    #     # If this error occurs, check for this number 2 or 3 or multiples.
    #     # This number needs to match with the size of  x = torch.cat((x, cond_emb1), dim=1) in the forward pass.
    #     self.combine_conv = torch.nn.Conv2d(self.ch*2, self.ch, kernel_size=1, stride=1, padding=0)
    ...
    #     print(f"AQUI ESTOY")
    #     assert (x.shape[2], x.shape[3]) == (256,512), \
    #    f"Input image shape {(x.shape[2], x.shape[3])} doesn't match expected {self.resolution}"
    #     # timestep embedding
    #     temb = get_timestep_embedding(t, self.ch)
    #     temb = self.temb.dense[0](temb)
    #     temb = nonlinearity(temb)
    #     temb = self.temb.dense[1](temb)
    #     print(f"Input to conv_in shape: {x.shape}")

    #     x = self.conv_in(x)

    #     if dx is not None:
    #         cond_emb1 = self.emb_conv(dx)
    #     else:
    #         cond_emb1 = torch.zeros_like(x)
    ...
    #     # if dx is not None:
    #     #     cond_emb2 = self.emb_conv(dx)
    #     # else:
    #     #     cond_emb2 = torch.zeros_like(x)
    #     # x = torch.cat((x, cond_emb2, cond_emb1), dim=1)

    #     x = torch.cat((x, cond_emb1), dim=1)
